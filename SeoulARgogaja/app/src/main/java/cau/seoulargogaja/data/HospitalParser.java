...
public class HospitalParser extends Thread {

    private String url;

    private ArrayList<String> id, name, latitude, longitude, address, phone;
    private ArrayList<HospitalDTO> list;


    public HospitalParser() {
        url = "https://cauteam202.com/hospital.xml";
    }
    ...
    public void run() {
        ...
        try {
            ...
            Log.e("hospital", "Error in network call", e);
        }

        Log.d("hospital", "크기 1: " + id.size());

        try {
            for (int i = 0; i < id.size(); i++) {  //사이즈 바꿔져야함!

                list.add(new HospitalDTO(id.get(i), name.get(i) ,latitude.get(i), longitude.get(i), address.get(i),phone.get(i)));
            }

        } catch (Exception e) {
            Log.e("hospital", "array null ", e);
        }
    }

    public ArrayList getList() {
        if(list == null) {
            Log.d("hospital","ArrayList의 값이 null입니다.");
        } else {
            Log.d("hospital","파싱완료");
        }
        return list;
